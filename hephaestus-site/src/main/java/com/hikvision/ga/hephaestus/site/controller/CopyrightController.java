...
public class CopyrightController {
  ...
  @ResponseBody
  public ListResult getListByPage(Integer pageNum, Integer pageSize, String sortBy,
      Integer direction) {
  ...
  public BaseResult saveCopyright(HttpServletRequest request) {
    ...
    if (copyrightService.findBySoftwareName(nameStripped).size() > 0) {
      ...
      OperationLogBuilder.build().operateResult(0).errorCode(RetStatus.NAME_DUPLICATED.getCode()).log();
      return res;
    }
   ...
   * @throws Exception
   */
  private Copyright setCopyrightProperties(HttpServletRequest request, Copyright c)
      throws Exception {
    if (null == c) {
      c = new Copyright();
    }
  ...
  @ResponseBody
  public ListResult searchCopyright(Integer field, String keyword, Integer pageNum,
    ...
    try {
      Page<Copyright> p =
          copyrightService.searchCopyrightByPage(fieldName, keywordList, pn, ps, sortBy, dir);
      if (null != p) {
        res.setListContent(
            new ListContent(p.getSize(), p.getTotalElements(), p.getTotalPages(), p.getContent()));
      }
