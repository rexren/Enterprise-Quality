 ...
 * @description FileUploadService Service in enterprise-quality
 */

angular.module('enterprise-quality').service(
        'FileUploadService',
        function () {
            ...
            function uploadFile(uploadBlock, file, input, uploadUrl, defer,
                    datas) {
                ...
                if (xhr.upload) {
                    xhr.upload.onprogress = function (_evt) {
                        if (_evt.lengthComputable) {
                            var progress = parseInt(_evt.loaded
                                    / upFile.getSize() * 100, 10);
            ...
            function openFiles(uploadBlock, uploadUrl, defer, datas, accept) {
                ...
                var input = createInput(uploadAttachInputId, {
                    accept : accept
                });
                ...
                input.change(function (_event) {
                    ...
                    // 验证通过之后
                    uploadFile(uploadBlock, files[0], input, uploadUrl, defer,
                            datas);
                    // 重置一下input
                    try {
                        _event.target.value = '';
